...
(function() {
  ...
  describe('Problem', function() {
    ...
    describe('check', function() {
      ...
      it('tests if all the capa buttons are disabled while checking', function(done) {
      ...
      return it('tests the expected change in text of check button', function(done) {
        var deferred, runs, self;
        deferred = $.Deferred();
        self = this;
        runs = function() {
          ...
          this.problem.check();
          expect(this.problem.checkButtonLabel.text).toHaveBeenCalledWith('Checking...');
          if (jQuery.active === 0) {
            deferred.resolve();
          }
          return deferred.promise();
    ...
    describe('check button on problems', function() {
      beforeEach(function() {
        this.problem = new Problem($('.xblock-student_view'));
        return this.checkDisabled = function(v) {
          return expect(this.problem.checkButton.hasClass('is-disabled')).toBe(v);
        };
      });
      describe('some basic tests for check button', function() {
      ...
      return describe('some advanced tests for check button', function() {
        it('should become enabled after a checkbox is checked', function() {
          ...
          this.problem.checkAnswersAndCheckButton(true);
    ...
    describe('reset', function() {
      ...
      return it('tests if all the buttons are disabled and the text of check button remains same while resetting', function(done) {
        var deferred, runs, self;
        deferred = $.Deferred();
        self = this;
        runs = function() {
          ...
          expect(this.problem.checkButtonLabel).toHaveText('Check');
          if (jQuery.active === 0) {
            deferred.resolve();
          }
          return deferred.promise();
        ...
        return runs.call(self).then(function() {
          ...
          return expect(self.problem.checkButtonLabel).toHaveText('Check');
        }).always(done);
      });
    });
    describe('show', function() {
      ...
      describe('when the answer has not yet shown', function() {
        ...
        it('reads the answers', function(done) {
          var deferred, runs;
          deferred = $.Deferred();
          runs = function() {
            spyOn($, 'postWithPrefix').and.callFake(function(url, callback) {
    ...
    describe('save', function() {
      ...
      it('reads the save message', function(done) {
        var deferred, runs;
        deferred = $.Deferred();
        runs = function() {
          spyOn($, 'postWithPrefix').and.callFake(function(url, answers, callback) {
      ...
      return it('tests if all the buttons are disabled and the text of check button does not change while saving.', function(done) {
        var deferred, runs, self;
        deferred = $.Deferred();
        self = this;
        runs = function() {
